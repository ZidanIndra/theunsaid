...
const getNicknameFromId = (authorId, fallbackName) => {
...
export default function BrowsePage({ publicEntries = [] }) {
  ...
  const userCards = useMemo(() => {
    ...
    filteredEntries.forEach((entry) => {
      ...
      const nickname =
        entry.author ||
        getNicknameFromId(entry.authorId || entry.author, t("unknown_user"));
  ...
  const headerLabel = useMemo(() => {
    const trimmed = query.trim();
    if (!trimmed) return t("browse_title_default");
    if (trimmed.includes("#")) {
      return t("browse_title_user_id", { id: trimmed });
    }
    return t("browse_title_user", { name: trimmed });
  }, [query, t]);
            ...
            {visibleCards.map((card) => (
              ...
              >
                <span className="inline-flex w-fit items-center rounded-full bg-zinc-200/10 px-3 py-1 text-xs text-zinc-300">
                  {t("browse_from_label", { name: card.nickname })}
                </span>
